...
std::vector<uint> addBasilLeaf( helios::Context* context ){
  ...
  for( int j=0; j<submesh_res.y; j++ ){
    for( int i=0; i<submesh_res.x; i++ ){
      ...
      UUIDs.push_back( context->addTriangle( v0, v1, v2, "../textures/BasilLeaf.png", uv0, uv1, uv2 ) );
      UUIDs.push_back( context->addTriangle( v0, v2, v3, "../textures/BasilLeaf.png", uv0, uv2, uv3 ) );
...
int main( void ){
  ...
  for( int t=0; t<Nt; t++ ){
	...
	context.addPatch( make_vec3(-0.5*Ntile*dx_tile+i*dx_tile,-0.5*Ntile*dx_tile+j*dx_tile,0), make_vec2(dx_tile,dx_tile), make_SphericalCoord(0,0), "../ConcreteTexture.jpg" );
    ...
    char filename[50];
    sprintf(filename,"../frames/basil_%03d.jpeg",t);
