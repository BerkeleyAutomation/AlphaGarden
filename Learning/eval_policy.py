...
def evaluate_analytic_policy_serial(wrapper_sel, env, policy, collection_time_steps, sector_rows, sector_cols, 
                            prune_window_rows, prune_window_cols, garden_step, water_threshold,
    ...
    for i in range(collection_time_steps):
        ...
        if wrapper and wrapper_day_set and ((i // sector_obs_per_day) >= PRUNE_DELAY):
            # At every new day, the wrapper runs through the day's garden to find the best prune rate and irrigaiton level for that day
            if i % sector_obs_per_day == 0:
                pr = 0
    ...
    for i in range(args.tests):
        ...
        if args.policy == 'ba':
            if args.multi:
                ...
                evaluate_analytic_policy_serial(False, env, analytic_policy.policy, collection_time_steps, sector_rows, sector_cols,
                                        prune_window_rows, prune_window_cols, garden_step, water_threshold,
                                        sector_obs_per_day, trial, save_dir, vis_identifier)
        elif args.policy == 'bw':
            if args.multi:
                ...
                evaluate_analytic_policy_serial(True, env, analytic_policy.policy, collection_time_steps, sector_rows, sector_cols,
                                        prune_window_rows, prune_window_cols, garden_step, water_threshold,
                                        sector_obs_per_day, trial, save_dir, vis_identifier)
        elif args.policy == 'n':
