...
def augment_soil_moisture_map(water_grid, trial, current_day, max_days, s1_avg, s2_avg, s3_avg, s4_avg, s5_avg, s6_avg, s1_pavg, s2_pavg, s3_pavg, s4_pavg, s5_pavg, s6_pavg):
    ...
    if s1 == None or current_day < start_buffer or current_day > max_days - end_buffer:
        ...
        np.savetxt("policy_metrics/water_grid/water_grid_" + str(current_day) + ".txt", np.array(water_grid), fmt="%f")
