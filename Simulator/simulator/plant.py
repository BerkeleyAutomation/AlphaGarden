    ...
    def from_preset(name, row, col):
        ...
        if name in PLANT_TYPES:
            ...
            return Plant(row, col, c1=p['c1'], growth_time=growth_time, max_radius=p['r_max'],
                         start_radius=p['start_radius'], germination_time=germination_length, color=p["color"],
                         plant_type=p["plant_type"], stopping_color=p["stopping_color"], color_step=p["color_step"])
        else:
