...
import numpy as np
from simulator.sim_globals import MAX_WATER_LEVEL, PRUNE_DELAY, PRUNE_THRESHOLD, PRUNE_RATE, IRR_THRESHOLD, \
	NUM_PLANT_TYPES_USED
...
def calc_potential_entropy( global_cc_vec, plants, sector_rows, sector_cols, prune_window_rows,
	...
	for i in range(1, len(global_cc_vec)):
		# Get cc for plants in prune sector
		if plant_in_area(plants, (sector_rows - prune_window_rows) // 2, (sector_cols - prune_window_cols) // 2,
						 prune_window_rows, prune_window_cols, i):
			prune_window_cc[i] = prune_window_cc.get(i, 0) + 1
	...
	prob = global_cc_vec[1:] / np.sum(global_cc_vec[1:],
									  dtype="float")  # We start from 1 because we don't include earth in diversity
	prob = prob[np.where(prob > 0)]
	entropy = -np.sum(prob * np.log(prob), dtype="float") / np.log(20)
	return proj_plant_cc, entropy
