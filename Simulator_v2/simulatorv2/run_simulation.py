...
def run_simulation(args):
    ...
    for i in range(NUM_TIMESTEPS):
        state = garden.get_garden_state()
        plants = garden.perform_timestep(water_amt=daily_water, irrigations=irrigation_policy(state, garden.step,
                                                                                              garden.irr_threshold, i))
...
def get_parsed_args():
    ...
    parser.add_argument('--irrigator', type=str, help='[uniform|sequential|baseline] The irrigation policy to use')
