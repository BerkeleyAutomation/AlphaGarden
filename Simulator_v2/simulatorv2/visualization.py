...
def _setup_plot(garden):
    ...
    major_ticks = np.arange(0, (garden.N - 1) * garden.step + 1, max(garden.N // 5, 1))
    minor_ticks = np.arange(0, (garden.N - 1) * garden.step + 1, garden.step)
...
def _add_plots(garden, ax, reverse=False):
    ...
    for plant in sorted(garden.plants.values(), key=lambda plant: plant.height, reverse=reverse):
        circle = plt.Circle((plant.row * garden.step, plant.col * garden.step), plant.radius, color=plant.color)
    ...
    #     if grid_pt['nearby']:
    #         circle = plt.Circle((coord[0] * garden.step, coord[1] * garden.step), 0.2, color='c', alpha=0.3)
    #         circleplot = ax.add_artist(circle)
    #         shapes.append(circleplot)
