...
def create_mask(leaf, leaf_type):
    ...
    """
    leaf[np.where((leaf >= [240, 240, 240]).all(axis=2))] = [0, 0, 0]
...
def build_randomized_layout(leaves_src, background_src, background_mask_src, augment_offset, background_dim, num_iters=15, seed=15):
    ...
    for i in random.sample(list(range(len(leaves))), num_iters):
        ...
        dim_ratio = np.random.uniform(0.7, 1.3)
...
if __name__ == "__main__":
    try:
        ...
        enc = config["convert_leaf_name"]
        leaf_ids = ([["leaves/" + f_name, enc[f_name[0:f_name.find("_")]]] for f_name in os.listdir("leaves") if ".jpg" not in f_name])
        print((leaf_ids))
        leaves = leaf_ids
        ...
        for i in range(num_simulations):
            ...
            plt.imsave("{}/1_{}-{}.jpg".format(original_folder_name, "original", i + offset), background_patch)
            plt.imsave("{}/1_{}-{}.png".format(original_mask_folder_name, "original", i + offset), background_mask_patch)
            plt.imsave("{}/1_{}-{}.jpg".format(generated_folder_name, "synthesized", i + offset), synthesized_background)
            plt.imsave("{}/1_{}-{}.png".format(generated_folder_name, "synthesized", i + offset), synthesized_mask)
